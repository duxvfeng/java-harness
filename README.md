...
- **🧠 智能推荐**: 基于任务特征自动推荐执行模式（Phase 13 新功能）
...
**Phase 13 新功能**: 基于任务特征自动推荐最优执行模式，消除 Solo/Parallel/Breezing 选择困惑！

Java Harness 现在支持智能执行模式推荐，通过分析任务四维度特征自动匹配最佳执行方式：

#### 核心特性
- ✅ **智能分析**: 基于任务数量、复杂度、依赖关系、审查需求四维度自动评估
- ✅ **透明决策**: 提供推荐理由和置信度评分，让用户理解选择依据
- ✅ **自动确认**: 高置信度（≥80%）推荐自动应用，减少决策负担
- ✅ **学习能力**: 记录用户反馈，持续优化推荐算法
- ✅ **LRU 缓存**: 推荐结果缓存，避免重复计算
...
任务描述 + 变更文件
       ↓
  [1. TaskAnalyzer] → 任务特征 (数量、复杂度、依赖、审查需求)
       ↓
  [2. ModeScorer]   → 模式评分 (Solo/Parallel/Breezing 各 0.0-1.0)
       ↓
  [3. RecommendationGenerator] → 推荐结果 (模式、置信度、理由、备选)
```
...
# 推荐: SOLO (置信度: 90%) - 单任务，低复杂度

# 场景 2: 3 个独立任务 → 推荐 PARALLEL
/harness-work --auto-mode 3-5
# 推荐: PARALLEL (置信度: 75%) - 多任务可并行

# 场景 3: 6 个复杂任务 → 推荐 BREEZING
/harness-work --auto-mode all
# 推荐: BREEZING (置信度: 85%) - 需要团队协作
```

...
description-ja: "团队执行模式 — harness-work 的团队协调别名。breezing、团队执行、全部完成、composer、作曲器、composer 2.5 时触发。"
...
**此技能委托给 `harness-work`**。请使用以下配置执行 `harness-work`：

1. **将参数原样传递给 `harness-work`**
2. **强制团队执行模式** — Lead → Worker spawn → Reviewer spawn 的三者分离
3. **Lead 专注于 delegate** — 不直接编写代码
4. **Auto Mode 作为 opt-in 处理** — `--auto-mode` 作为兼容父会话中的 rollout 用标志接受
5. **Advisor 仅在必要时** — 仅在 Worker 返回 `advisor-request.v1` 时 Lead 调用 advisor

### Plan-time 事前确认的处理

Breezing run 开始时，Lead 执行与 `harness-work` 相同的 preapproval preflight。

- 各任务开始时，在 task worktree 的 `.claude/state/active-task.json` 中原子地写入 `{"phase":"<phase>","task":"<task>"}`。任务结束时在成功、失败、停止的所有路径中删除。
- 如果存在 `.claude/state/plan-preapprovals.json` 则用 `scripts/plan-preapproval.sh validate` validate v2。v1 作为现有记录的读取兼容接受。
- 仅将执行对象的 task 的 `decision: approved` 事项作为已声明处理，传递给 Worker briefing。
- `secret-read` 通过 `bash "${HARNESS_PLUGIN_ROOT}/scripts/plan-preapproval.sh" apply-secret-allow "$PROJECT_ROOT"` 将 project config `.claude-code-harness.config.json` 的 `runtimefloor.secretAllow` 反映为 per-run，与 108.2 的 project config floor 连接。
- R12 的 `ask` 仅抑制同一 phase/task、期限内、使用次数内与 `external-send` 和执行命令一致的 v2 承认。不抑制明确 `deny` 和 runtime floor。
- 已声明事项不中途停止，工作中的已声明事项起因的 `AskUserQuestion` 归零。确认仅在 plan 承认时的 1 次。
- 记录中没有的未计划的 secret-read / 外部发送 / 破坏性操作照旧通过 runtime floor / ask 停止。不缩小安全网。

### 与 `harness-work` 的不同

| 特征 | `harness-work` | `breezing` (此技能) |
|------|-----------------|------------------------|
| 并行手段 | 根据必要数的自动分割 | **Lead/Worker/Reviewer 的角色分离** |
...
> *父会话或 frontmatter 为 `bypassPermissions` 时那边优先。分发模板现在也使用 `bypassPermissions`，Auto Mode 是 follow-up 的 rollout 对象，不是默认行为。

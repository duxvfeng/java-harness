...
description-zh: "让 Cursor Composer 作为咨询性第二意见进行审查的技能。当用户提到 cursor:review、让 Cursor 审查、让 composer 检查差异时使用。主要审查结论始终由宿主大脑持有，Cursor 不拥有 APPROVE/REQUEST_CHANGES 权限。"
...
将 Cursor Composer (`composer-2.5-fast`) 作为 read-only second opinion 使用的 review skill。primary verdict 由 host brain (Opus / Claude role) 发出。

...
将单个实现任务委托给 Cursor Composer (`composer-2.5-fast`)，在专用 worktree 内执行，Lead 审查 diff 后 cherry-pick 到 main 的技能。不启动 breezing 的 team 流程，以最短路径完成 1 任务 1 cherry-pick。

Cursor 侧没有封存 ([references/cursor-cli-only.md](${CLAUDE_SKILL_DIR}/references/cursor-cli-only.md))。**专用 `.git` 的 worktree + Lead diff review + cherry-pick (R01-R13 路径)** 三点才是实际边界。cursor 的输出在 Lead 审查前视为 untrusted。

## Step 0 — NARRATION RULES (UX Contract)

敌人是**冗长**，而不是进度报告。与 breezing 相同契约。**启动时简洁明了地输出 banner + 执行计划，然后执行**。欢迎易读的进度报告，仅禁止冗长重复。

### 启动时必须输出 (banner + plan，总计 5 行以内)

```
🚀 cursor / composer-2.5-fast / feat/foo-bar / Add login form validation
接下来:
1. pre-check (branch / cursor-agent) → 创建专用 worktree
2. 向 composer 委托实现 (--write)
3. diff 审查 → cherry-pick → Plans.md 更新
```

banner 1 行 (`🚀 cursor / composer-2.5-fast / <branch> / <task>`) + 计划 2-4 行。1 秒内输出，立即进入 Step 1。

### 可以输出进度报告 (易读范围内)

- 各步骤的开始・完成以 1 行状态 (`✓ worktree 创建: .claude/worktrees/cursor-do-...`)
- pre-check / resolve 的要点、cherry-pick 的 SHA
- 为什么采取这个分支的理由用 1 行说明

### 禁止 (= 冗长)

- **同一事实的两次重述**: 不在后段重新说明 pre-check 结果
- **无内容的前言**: 仅 tool call 自明声明的行
- **3 行以上的经过回顾**: 必要时压缩为 1 行
- **启动序列中的 ★ Insight 块**: Insight 仅在最终 report 中出现一次

违反例 (冗长):
```
× "用 composer 2.5 实现的流程呢，首先确认"（无内容前言）
× "调用 Cursor 前查看 branch" → bash → "确认了 branch"（重述）
× ★ Insight ──── Cursor 的优势是…
```
...
```
🚀 cursor / composer-2.5-fast / feat/foo-bar / Add login form validation
接下来: worktree 创建 → 向 composer 委托实现 → diff 审查 → cherry-pick
```

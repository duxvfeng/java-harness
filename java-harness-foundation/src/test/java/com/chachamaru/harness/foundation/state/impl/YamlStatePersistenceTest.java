 ...
 * YAML 状态持久化引擎测试
 */
class YamlStatePersistenceTest {
    ...
    @BeforeEach
    void setUp() {
        persistence = new YamlStatePersistence<>();
        testFile = tempDir.resolve("test-state.yaml");
    }
    ...
    void testSaveAndLoadComplexState() throws PersistenceException {
        ...
        // 使用类型转换的持久化引擎
        StatePersistenceEngine<ComplexState> complexPersistence = new YamlStatePersistence<>();
    ...
    void testCreateDirectoryIfNeeded() throws PersistenceException {
        Path nestedFile = tempDir.resolve("nested").resolve("dir").resolve("state.yaml");
    ...
    void testThreadSafety() throws PersistenceException, InterruptedException {
            ...
            threads[i] = new Thread(() -> {
                try {
                    for (int j = 0; j < OPERATIONS_PER_THREAD; j++) {
                        ...
                        Path threadFile = tempDir.resolve("thread-" + threadId + ".yaml");
        ...
        // 验证所有线程都成功完成了操作
        for (int i = 0; i < THREAD_COUNT; i++) {
            Path threadFile = tempDir.resolve("thread-" + i + ".yaml");
            assertTrue(persistence.exists(threadFile), "Thread file should exist");
        }
    }

    @Test
    void testYamlFormat() throws PersistenceException {
        TestState state = new TestState();
        state.setName("yaml-format-test");
        state.setValue(99);
        ...
        Optional<TestState> loaded = persistence.load(testFile, TestState.class);
        assertTrue(loaded.isPresent());
        assertEquals("yaml-format-test", loaded.get().getName());
        assertEquals(99, loaded.get().getValue());
        assertFalse(loaded.get().isEnabled());
    }

    @Test
    void testSupportYmlExtension() throws PersistenceException {
        Path ymlFile = tempDir.resolve("test-state.yml");

        TestState state = new TestState();
        state.setName("yml-extension-test");
        state.setValue(77);

        persistence.save(state, ymlFile);

        Optional<TestState> loaded = persistence.load(ymlFile, TestState.class);
        assertTrue(loaded.isPresent());
        assertEquals("yml-extension-test", loaded.get().getName());
    }

...
## Phase 17: harness-work 分支隔离状态持久化优化（用户体验改进）🆕

### 📋 需求说明

本 Phase 优化 `/harness-work` 的分支隔离交互机制，实现更智能的状态管理：
- **交互提示**: 在执行 `/harness-work` 时提供清晰的交互选项，让用户选择是否创建分支和隔离区
- **状态保持**: 一旦创建分支隔离区，该系列任务无需再次创建，避免重复提示
- **智能重置**: 当分支隔离区代码提交清空后，自动重置状态，下次任务重新提示
- **状态可见性**: 用户能清晰看到当前的隔离状态和决策历史

### 事前确认章节

**Phase 17 涉及状态文件管理和 Git 状态检测，需要在计划批准时预先确认以下事项：**

#### 计划时批准事项

- **事项**: 读取 Git 状态（分支、worktree、提交状态）
  - **理由**: Task 17.1-17.4 需要读取当前分支状态、worktree 信息、提交历史
  - **scope**: Phase 17 / Task 17.1-17.4

- **事项**: 文件系统写入（状态文件）
  - **理由**: Task 17.3-17.5 需要创建和更新 `.claude/state/` 下的状态文件
  - **scope**: Phase 17 / Task 17.3-17.5

#### 说明
- Git 状态读取仅用于检测分支和提交情况，不修改代码
- 状态文件记录用户决策和隔离状态，不包含敏感信息
- 与现有 Phase 10 的分支隔离功能向后兼容

---

| Task | 内容 | DoD | Depends | Status |
|------|------|-----|---------|--------|
| 17.1 | 分析当前分支隔离状态机制 `[tdd:skip:analysis]` | 分析 Phase 10 的实现，识别状态管理的问题和改进点 | 分析报告完成，问题清单明确，改进方向清晰 | - | cc:完成 ✅ 2026-08-13 [分析报告] |
| 17.2 | 设计状态持久化架构 `[tdd:skip:design]` | 设计隔离状态的数据模型、生命周期、重置条件、检测机制 | 架构设计文档完成，包含状态转换图、重置条件定义 | 17.1 | cc:完成 ✅ 2026-08-13 [架构文档] |
| 17.3 | 扩展状态文件模型 `[tdd:required]` | 扩展 `.claude/state/branch-isolation-decision.json`，添加任务序列跟踪、代码状态跟踪、重置触发器 | 状态文件格式正确，包含所有必需字段，JSON 序列化正常 | 17.2 | cc:完成 ✅ 2026-08-13 [v2 JSON 往返、派生字段排除、旧格式迁移] |
| 17.4 | 实现代码状态检测器 `[tdd:required]` | 创建 CodeStatusDetector 类，检测分支中的代码状态（是否有未提交的更改） | 能准确检测分支代码状态，区分"有代码"和"无代码"状态，测试通过 | 17.3 | cc:完成 ✅ 2026-08-13 [真实 Git worktree、修改/新增/删除文件、UTC 时间、base ref] |
| 17.5 | 实现状态重置逻辑 `[tdd:required]` | 创建 IsolationStateReset 类，实现自动重置和手动重置功能 | 重置逻辑正确，能准确识别重置时机，测试通过 | 17.4 | cc:完成 ✅ 2026-08-13 [完成/不活跃且干净分支触发，保留审计历史] |
| 17.6 | 改进用户交互界面 `[tdd:required]` | 改进 `/harness-work` 的交互提示，显示当前状态、提供清晰选项、说明后续影响 | 交互提示清晰，用户能理解当前状态和选择结果，测试通过 | 17.5 | cc:完成 ✅ 2026-08-13 [同系列任务自动继续，跨系列重新决策] |
| 17.7 | 集成到 harness-work 流程 `[tdd:required]` | 修改 harness-work SKILL.md，在 Phase A 集成新的状态管理和交互逻辑 | 状态管理工作正常，交互提示正确，与现有流程兼容，测试通过 | 17.6 | cc:完成 ✅ 2026-08-13 [Java Phase A 入口与 Shell v2 兼容适配入口统一] |
| 17.8 | 编写单元测试 `[tdd:required]` | 为状态检测、重置逻辑、交互组件编写单元测试 | 测试覆盖率 > 80%，所有核心逻辑有测试 | 17.7 | cc:完成 ✅ 2026-08-13 [19 个专项测试，覆盖持久化、迁移、Git、重置和决策语义] |
| 17.9 | 编写集成测试 `[tdd:required]` | 测试完整的状态生命周期：创建→保持→重置→重新提示 | 集成测试覆盖所有场景，测试通过 | 17.8 | cc:完成 ✅ 2026-08-13 [真实仓库生命周期：创建→同系列保持→完成→自动重置→新系列提示] |
| 17.10 | 更新用户文档 `[tdd:skip:docs-only]` | 更新 README.md 和相关文档，说明新的状态管理机制和用户交互 | 文档包含使用说明、状态转换图、故障排除 | 17.9 | cc:完成 ✅ 2026-08-13 [v2 状态格式、入口约定、重置条件和故障排除] |
...
1. **智能状态保持**: 记住分支隔离创建决策，避免重复提示
2. **自动重置**: 检测分支代码状态，自动判断是否应该重置隔离状态
3. **清晰交互**: 提供明确的选项和状态说明，让用户做出知情决策
4. **状态可见**: 用户能随时查看当前的隔离状态和决策历史

### 📊 工作范围

- **修改文件**:
  - `skills/harness-work/SKILL.md` - 改进交互提示和状态管理
  - `.claude/state/branch-isolation-decision.json` - 扩展状态文件格式

- **新增文件**:
  - `java-harness-workflow/src/main/java/com/chachamaru/harness/isolation/CodeStatusDetector.java`
  - `java-harness-workflow/src/main/java/com/chachamaru/harness/isolation/IsolationStateReset.java`
  - `docs/harness-project/user-guide/isolation-state-management.md` - 用户指南

### 🔧 状态管理机制

#### 状态定义
- **初始状态**: 用户未做任何决策，每次执行 `/harness-work` 都提示
- **已隔离状态**: 用户已创建分支和隔离区，同一系列任务不再提示
- **待重置状态**: 分支隔离区代码已提交清空，等待状态重置
- **重置状态**: 回到初始状态，下次任务重新提示

#### 重置条件
- **自动重置**: 检测到分支隔离区代码已提交且无未提交更改
- **手动重置**: 用户主动调用命令重置状态
- **任务完成**: 当前 Plans.md 中的所有任务完成后自动重置

### 🔄 与 Phase 10 的关系

- **扩展而非替代**: Phase 17 扩展 Phase 10 的功能，不重复已有实现
- **状态文件共享**: 使用同一个状态文件，添加新字段而不破坏兼容性
- **向后兼容**: 现有分支隔离功能继续正常工作
...
**推荐执行顺序**:
1. **立即开始** Phase 17（分支隔离状态持久化优化）- 解决用户提出的状态管理问题
2. 可选完成 **Phase 16**（配置文档和案例整理）- 改善文档体验
3. 可选完成 **Phase 15**（技能边界清晰化）- 架构优化

**启动方式**: 使用 `/harness-work` 开始实施 Phase 17
...
| Task | 内容 | DoD | Depends | Status | 完成标记 |
|------|------|-----|---------|--------|---------|
| 15.1 | 分析当前技能边界问题 `[tdd:skip:analysis]` | 分析 harness-plan 当前可能越界的场景，识别问题根源 | 分析报告完成，问题场景清晰，根源明确 | - | cc:完成 ✅ 2026-08-13 [分析报告] |
| 15.2 | 设计技能边界架构 `[tdd:skip:docs-only]` | 设计明确的技能边界定义，制定协作协议，创建边界检查清单 | 架构设计文档完成，包含边界定义、协作协议、检查清单 | 15.1 | cc:完成 ✅ 2026-08-13 [架构文档] |
| 15.3 | 更新 harness-plan SKILL.md `[tdd:skip:config-task]` | 在 harness-plan 中添加明确的边界声明，禁止直接修改代码，增加边界检查 | SKILL.md 包含边界章节，有明确的禁止操作列表，符合架构设计 | 15.2 | cc:完成 ✅ 2026-08-13 [边界声明] |
| 15.4 | 更新 harness-work SKILL.md `[tdd:skip:config-task]` | 在 harness-work 中明确其作为实现执行者的职责，与规划技能的交接标准 | SKILL.md 包含职责定义和交接标准，与 harness-plan 边界一致 | 15.2 | cc:完成 ✅ 2026-08-13 [职责定义] |
| 15.5 | 创建技能边界测试 `[tdd:required]` | 创建边界测试用例，验证各技能不会越界操作，检测异常行为 | 测试用例覆盖主要技能，能检测越界操作，测试通过 | 15.3 | cc:完成 ✅ 2026-08-13 [测试系统] |
| 15.6 | 更新用户文档和示例 `[tdd:skip:docs-only]` | 在文档中说明技能边界，提供正确使用示例，避免用户混淆 | 文档包含边界说明，使用示例清晰，用户能正确理解技能职责 | 15.4 | cc:完成 ✅ 2026-08-13 [用户指南] |
| 15.7 | 技能边界验证和发布 `[tdd:required]` | 运行完整测试套件，验证边界清晰化效果，确保无破坏性变更 | 所有测试通过，边界清晰化效果明显，无功能回归 | 15.5 | cc:完成 ✅ 2026-08-13 [验证报告] |

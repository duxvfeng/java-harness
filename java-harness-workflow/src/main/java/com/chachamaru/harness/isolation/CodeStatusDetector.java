...
public class CodeStatusDetector {
    ...
    private static final Pattern UNTRACKED_FILE_PATTERN = Pattern.compile("^\\?\\?\\s+(.+)$");
    private static final Pattern MODIFIED_FILE_PATTERN = Pattern.compile("^\\s*M\\s+(.+)$");
    private static final Pattern COMMIT_TIMESTAMP_PATTERN = Pattern.compile("^\\s*\\d+\\s+(\\d+)\\s+");
        ...
        try {
            ...
            if (result != null && !result.trim().isEmpty()) {
                ...
                for (String line : lines) {
                    Matcher matcher = MODIFIED_FILE_PATTERN.matcher(line);
                    if (matcher.find()) {
    ...
    private int countUntrackedFiles(String worktreePath) {
        ...
        try {
            ...
            if (result != null && !result.trim().isEmpty()) {
                String[] lines = result.split("\n");
                for (String line : lines) {
                    Matcher matcher = UNTRACKED_FILE_PATTERN.matcher(line);
                    if (matcher.find()) {
                        count++;
                    }
                }
    ...
    private String executeGitCommand(String worktreePath, String... commands) throws Exception {
        ...
        fullCommand.add("--git-dir=" + worktreePath + "/.git");
        fullCommand.add("--work-tree=" + worktreePath);
    ...
    private static class GitCommitInfo {
        ...
        public LocalDateTime getTimestamp() {
            return timestamp != null ?
                LocalDateTime.ofEpochSecond(timestamp / 1000, 0, null) : null;
        }
    ...
    public boolean isValidGitWorktree(String worktreePath) {
        try {
            File gitDir = new File(worktreePath, ".git");
            if (!gitDir.exists()) {
                return false;
            }

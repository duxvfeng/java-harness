...
public class IsolationStateReset {
    ...
    public ResetEvaluation evaluateResetConditions(IsolationStateFile state, CodeStatus codeStatus) {
        ...
        // A clean branch is a prerequisite for automatic reset, not a reset
        // trigger by itself. Completion or inactivity must also be established.
        // Condition 1: Task series is complete
        if (isTaskSeriesComplete(state, codeStatus)) {
            satisfiedConditions.add("task_series_complete");
        }

        // Condition 2: Inactivity timeout with clean branch
